...
The context backend is optional. It helps AppForge retrieve, search, and validate project memory for larger apps.

Default mode remains file/GitHub/project-control only.
...
The default context mode is a local JSONL index. Qdrant, Supabase pgvector, and generic vector DB profiles are optional upgrades.

The context backend retrieves and indexes context. It is not the canonical source of truth. All retrieved items must point back to project-control, docs, Figma, GitHub repos, design-assets, or other approved sources.
