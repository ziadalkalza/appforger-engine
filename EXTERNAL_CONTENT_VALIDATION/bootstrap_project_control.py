...
def main():
    ...
    for b in BASELINES:
        write_once(pc/"BASELINES"/b, f"# {b.replace('_',' ').replace('.md','').title()}\n\n")
    write_once(pc/"ARTIFACT_IMPORTS"/"IMPORT_INBOX.md", "# Import Inbox\n\nUse this to register incoming Figma, image, code, QA, and backend artifacts before approval.\n")
    write_once(pc/"CODEX_WORK_PACKETS"/"README.md", "# Codex Work Packets\n\nGenerated Codex task packets for this app live here.\n")
    write_once(pc/"DECISION_SNAPSHOTS"/"STAGE_SUMMARY.md", "# Stage Summary\n\nKeep a compact summary of the current stage here.\n")
