...
All reusable templates/examples are stored inside the engine under `ENGINE_TEMPLATES/` and `EXAMPLES/`. The initializer copies/scaffolds from those templates into a real workspace.
...
Before starting real project work in a project that has not completed onboarding, use `PROJECT_ONBOARDING_AND_INITIALIZATION/`.
...
Use `MODEL_AND_TOOL_AGNOSTIC_PROVIDER_LAYER/` and `EXECUTION_PACKETS/` for provider selection and handoff.


## Parallel work and sub-agent orchestration

For parallel work, use `PARALLEL_WORK_AND_SUB_AGENT_ORCHESTRATION/`. A sub-agent is a role-scoped worker executed by a provider, human, or future AppForge service.
...
Do not add external packages casually. Before recommending or adding a package/library, use `PACKAGE_AND_LIBRARY_DECISIONS/` and record the decision in `PACKAGE_DECISION_REGISTRY.md` inside project-control.
...
6. `START_HERE/STAGE_WORKFLOW.md`
7. `START_HERE/HUMAN_APPROVAL_GATES.md`
8. `START_HERE/ITERATION_AND_CHANGE_REQUESTS.md`
9. `START_HERE/CONTEXT_KEEPING_RULES.md`
...
AppForge may use external design systems, design `.md` files, Figma UI kits, brand guides, or visual critique skills, but they must be imported through `DESIGN_SYSTEM_IMPORTS/` and registered in `REGISTRIES/IMPORTED_DESIGN_SYSTEM_REGISTRY.md` before use.
...
4. `DECISION_LOG.md`
5. `PENDING_QUESTIONS.md`
6. `DECISION_SNAPSHOTS/PROJECT_MEMORY_COMPACTION.md`

Then load only the current stage contract, relevant skill, active baseline, and task packet.

Use `CONTEXT_AND_TOKEN_MANAGEMENT/` for token and context rules. In particular:
...
- affected baselines in `BASELINES/`
...
Never create production app code inside the AppForge engine folder. If asked to build backend, iOS, Android, or design-assets code, first identify the correct standalone repo from `REGISTRIES/REPO_REGISTRY.md` and the workspace layout from `WORKSPACE_SCAFFOLD/WORKSPACE_LAYOUT.md`.
...
For development tasks, create or use a provider-agnostic execution packet from `project-control/EXECUTION_PACKETS/`. The execution packet is the bridge between AppForger orchestration and code/design/tool providers. Codex-specific packet templates are adapters, not the generic packet model.

For artifacts returned from Figma, image generation, GitHub, backend, web, mobile, iOS, or Android repos, use `ARTIFACT_IMPORTS/IMPORT_INBOX.md` and the relevant import checklist before changing baselines or registries.

Use baseline versioning from `BASELINES/` for approved design, API, backend, and frontend states. A later approved baseline supersedes, but does not erase, earlier baselines.
...
- `CONTEXT_AND_TOKEN_MANAGEMENT/WEB_CONTEXT_PACK.md`
- `WEB_FRONTEND/` selected stack rules
- `FIGMA_TO_WEB_MAPPING/` mapping files
...
Before writing an artifact, classify its lifecycle using `WORKSPACE_AND_ARTIFACT_LIFECYCLE/ARTIFACT_LIFECYCLE_POLICY.md`.
...
- `EXTERNAL_CONTENT_VALIDATION/REQUIRED_WORKSPACE_STRUCTURE.md`
- `EXTERNAL_CONTENT_VALIDATION/STAGE_DEPENDENCY_MATRIX.md`
- `EXTERNAL_CONTENT_VALIDATION/validate_external_workspace.py`

If `project-control/` is missing or incomplete, bootstrap or repair it using:

- `EXTERNAL_CONTENT_VALIDATION/bootstrap_project_control.py`
- `WORKSPACE_AND_ARTIFACT_LIFECYCLE/PROJECT_CONTROL_BOOTSTRAP_POLICY.md`
...
- `REMOTE_AND_MULTI_DEVICE_WORKFLOWS/LOCAL_VS_REMOTE_CONTEXT_POLICY.md`
- `REMOTE_AND_MULTI_DEVICE_WORKFLOWS/REMOTE_REPO_ACCESS_POLICY.md`
- `REMOTE_AND_MULTI_DEVICE_WORKFLOWS/GITHUB_CONTEXT_FETCH_PROTOCOL.md`
- `REMOTE_AND_MULTI_DEVICE_WORKFLOWS/PROJECT_CONTROL_FRESHNESS_POLICY.md`
- `EXTERNAL_CONTENT_VALIDATION/validate_remote_context.py`
...
Use `TASK_COMPLETION_AND_EVIDENCE/` to classify task state.
...
Use `PLAN_MODE_AND_UNCERTAINTY/` for ambiguity, risk, and token-saving behavior.
...
- `SKILLS/engine_upgrade_skills/create_integration_pack/SKILL.md`
...
Track lifecycle state using `CONNECTOR_MCP_SKILL_LIFECYCLE/` and update:
...
Before declaring AppForge stable after major feature additions, run the operational audit layer in `OPERATIONAL_AUDIT/`. The audit covers the full accumulated engine, not only the latest version. Apply safe wording/structure fixes automatically only when they do not change behavior. Ask the user before behavior-changing fixes.
