...
Appforger must classify task status honestly without blocking normal development flow.
...
If required evidence is missing, Appforger must not reject the user's decision. It must record the correct status and risk:

- `user_marked_done_needs_evidence`
- `completed_with_warnings`
- `manually_closed`

Appforger must create follow-up validation tasks when missing evidence matters.
