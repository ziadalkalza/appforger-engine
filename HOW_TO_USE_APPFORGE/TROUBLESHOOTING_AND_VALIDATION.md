...
- Confirm the zip root contains only `appforge-engine/`.
...
3. Confirm `appforge-engine/` is next to, not inside, `backend/` or `web/`.
4. Run the validation command above.
5. Check `START_HERE/SETUP_AND_FIRST_RUN_GUIDE.md` and this documentation folder.
...
4. Check `RUNTIME_INFRASTRUCTURE/README.md`.

## If source pipelines fail

1. Check the source registry for the source ID.
2. Check the relevant skill under `SKILLS/source_pipeline_skills/`.
...
2. Check `APPFORGE_MCP_SERVER/INSTALL_AND_USE.md`.
