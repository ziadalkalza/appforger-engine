...
Before AppForge starts a stage that depends on external folders or services, it must validate that the external content exists and follows AppForge rules.

Validate:

- `project-control/` exists and contains required state files.
- selected implementation repos exist locally.
- selected implementation repos contain `AGENTS.md` and `APPFORGE_POINTER.md`.
