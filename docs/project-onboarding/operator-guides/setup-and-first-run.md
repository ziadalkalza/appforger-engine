...
Do not place `appforger-engine/` inside `backend/` or `web/`. It should sit beside project folders.
...
```text
Read appforger-engine/docs/project-onboarding/start-here/start-here-for-agents.md and set up Appforger for this project.
```
...
```bash
python appforger-engine/workflows/operations/new-app-initializer/setup_appforge_project.py \
  --target . \
  --name "My Product" \
