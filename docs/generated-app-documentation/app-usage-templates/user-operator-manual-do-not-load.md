# Appforger User Operator Manual — DO NOT LOAD AS AGENT CONTEXT
...
Appforger is an engine. You should not repeat all engine rules in every prompt.
...
Run the new app wizard, create the initial product brief, choose the app mode, identify design/backend/frontend decisions, and tell me which Appforger files should be updated.
...
```text
Use Appforger to start the [stage name] stage.
...
```text
Use Appforger to create an image-generation request packet for [logo/icon/onboarding illustration/empty state/app store screenshot].
...
```text
Use Appforger to review these generated image assets and prepare the approved ones for Figma import.
```

## 5. Working with Figma

Start in ChatGPT/Appforger:
...
```text
Mark this as DESIGN_BASELINE_V[number] and update Appforger memory.
```

## 7. Starting backend work

```text
Use Appforger to select the backend stack for this approved design.
...
Read this repo's AGENTS.md and APPFORGER_POINTER.md.
Use the supplied Codex work packet only.
Implement the task, run required checks, and produce a done report.
```

Do not ask Codex to read the whole Appforger folder.

## 9. After Codex finishes

Bring the result back to ChatGPT/Appforger:
...
Although Appforger should do this automatically at stage/task completion, you can force it:

```text
Save Appforger memory for what we just did.
Update APPFORGER_PROJECT_STATUS.md, affected registries, decisions, open questions, and create a compact summary for the next session.
```
...
```text
Record this GitHub update in Appforger memory.
Repo:
Branch:
Commit/PR:
Summary:
...
```text
Continue Appforger from the current stage.
...
```text
Enter Appforger plan mode. Do not write code or change files yet.
...
```text
Run Appforger integration upgrade for [SERVICE].
...
```text
Check the Appforger lifecycle status for [GitHub/Figma/Supabase/Codex skill/etc.].
