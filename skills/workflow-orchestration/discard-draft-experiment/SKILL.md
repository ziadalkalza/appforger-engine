# Discard Draft Experiment

## Purpose

Mark a draft as discarded while preserving useful learning.

## Required context

- `AGENTS.md`
- `workflows/artifact-lifecycle/draft-sandbox/README.md`
- `project-control/registries/DRAFT_REGISTRY.md` when available
- relevant epic/feature/story if known

## Rules

- Do not modify approved baselines directly.
- Do not write draft artifacts into `appforger-engine/`.
- Keep raw draft files in `local-only/` or a clearly marked draft branch.
- Update draft registries when the user wants the draft tracked.
- Ask before promotion.

## Steps

1. Confirm discard decision.
2. Record reason.
3. Mark draft registry as discarded.
4. Optionally create a short lesson/avoidance note.

## Output

Discarded draft record.
