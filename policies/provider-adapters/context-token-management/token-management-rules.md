...
Appforger must minimize token usage by loading context progressively.

## Non-negotiable rules

1. Do not read the entire Appforger folder for normal app work.
...
8. The user-facing operator manual marked `DO_NOT_LOAD` is not execution context and should not be read unless the user explicitly asks how to use Appforger.

## Default context layers

### Always-read operating context

Read only these at the start of a normal Appforger session:
