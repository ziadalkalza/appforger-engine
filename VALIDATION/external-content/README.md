...
This folder contains rules and scripts that verify the folders and external project artifacts Appforger depends on actually exist.

Appforger should never assume external project-control or code repos are present. It must validate them.
...
```bash
python validation/domains/external-content/validate_remote_context.py --workspace ./my-app-workspace --require ios,backend
```
