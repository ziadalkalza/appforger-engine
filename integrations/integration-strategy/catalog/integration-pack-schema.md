...
Every Appforger third-party integration pack must follow this schema. The schema tells Appforger what information to collect; the specific values must come from official docs, project constraints, and user answers.
...
1. `README.md` — what the integration is for and how Appforger uses it.
2. `BOUNDARY.md` — what the third-party tool does and does not own.
3. `CLARIFICATION_QUESTIONS.md` — questions Appforger must ask before using the service.
...
15. `REGISTRY_UPDATES.md` — Appforger/project-control files that must be updated.
