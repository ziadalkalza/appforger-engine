...
This folder defines how Appforger adds and uses third-party integrations such as Stripe, Firebase, Mapbox, Twilio, SendGrid, Sentry, RevenueCat, Rive, LottieFiles, AWS, or any other external service.

Appforger must not improvise unsupported integrations. A third-party tool becomes usable only after an integration pack exists, has been reviewed, registered, and activated for the project.
...
→ Appforger checks whether an integration pack exists
...
```text
Engine upgrade = teach Appforger how to use the integration safely and repeatably.
